...
function normalizePosition(pos) {
  ...
  if (['fsdsl','dsl','ds','fsds','fsdls'].includes(s)) return 'DSL';
...
function getHeroStatsFromGames(heroName, games) {
  ...
  return Object.entries(statsMap).map(([position,{win,total}])=>({
    position,total,win,
    pickRate: ((total/totalGames)*100).toFixed(1),
    winRate: total?((win/total)*100).toFixed(1):'0.0',
  }));
}

export default function DraftPage({
  games, comboStats, excelData, heroes = heroList,
}) {
  ...
  const filteredHeroList = useMemo(()=>{
    let list = selectedRole==='ทั้งหมด' ? heroes : heroes.filter(h=>normalizeRole(h.role)===selectedRole);
    if (searchTerm.trim()) {
  ...
  useEffect(() => {
    ...
    const teams = sheets.filter(n=>["TLN","BRU","PSG","KOG","HD","BAC","EA"].includes(n));
    setOpponentSheetList(teams);
    setOpponentSheet(teams.includes(opponentSheet)?opponentSheet:(teams[0]||''));
  },[excelData]);
  useEffect(()=>{
    if (!excelData||!opponentSheet) { setOpponentStats(null); return; }
    const sheet = excelData.Sheets[opponentSheet];
    if (!sheet) { setOpponentStats(null); return; }
  ...
  function renderTooltip(){
    ...
    return (
      <div style={{
        position:'fixed', left:tooltipPos.x+14, top:tooltipPos.y+12,
        background:'#181a23', border:`2.5px solid ${highlightColor}`, borderRadius:12,
        padding:12, color:'#fff', fontSize:14, zIndex:1001, pointerEvents:'none',
        boxShadow:'0 8px 24px #000a'
      }}>
        <div style={{fontWeight:900,fontSize:16,marginBottom:6,color:'#fff600'}}>{tooltip.heroName}</div>
        <div style={{marginBottom:6}}>
          <b>ฝั่งเรา</b>:<br/>
          {tooltip.ourList.length?tooltip.ourList.map((e,i)=>(
            <div key={i}>({e.position}) Total:{e.total}, Win:{e.win},<br/>PickRate:{e.pickRate}%, WinRate:{e.winRate}%</div>
          )):<span>- ไม่มีข้อมูล -</span>}
        </div>
        <div>
          <b>ฝั่งตรงข้าม ({opponentSheet})</b>:<br/>
          {tooltip.oppList.length?tooltip.oppList.map((e,i)=>(
            <div key={i}>({e.position}) Total:{e.total}, Win:{e.win},<br/>PickRate:{e.pickRate}%, WinRate:{e.winRate}%</div>
          )):<span>- ไม่มีข้อมูล -</span>}
        </div>
      </div>
  ...
  function renderHistoryView() {
    ...
    return (
      <div style={{ display:'flex', height:'100vh', background:'#161720', position:'relative' }}>
        ...
        <button onClick={()=>setViewingHistory(null)} style={{
  ...
  function renderSlots(arr, max, team, type) {
    return Array.from({ length: max }).map((_, i) => {
      ...
      if (type === 'pick') {
        ...
        if (hero) {
          return (
            <div key={i} style={{ margin:6, textAlign:'center', width:76, height:100,
          ...
          return (
            ...
            >
              ...
              <button onClick={()=>handleClearPreChoose(team,i)} title="ลบ pre-choose"
                style={{ position:'absolute', top:3, right:5, background:'transparent', border:'none',
                  color:'#FFD700', fontSize:16, cursor:'pointer' }}>×</button>
            </div>
          );
        } else {
          return (
  ...
  function renderLegend() {
    return (
      <div style={{
        ...
        background:'#1a1c27', borderRadius:10, boxShadow:'0 2px 10px #0008',
        justifyContent: align==='left' ? 'flex-start' : 'flex-end'
      }}>
        <div style={{ color:'#9fb0ff', fontWeight:900 }}>{label}</div>
        <div style={{ display:'flex', gap:6 }}>
          {[...bansArr, ...Array(Math.max(0,4-bansArr.length)).fill(null)].slice(0,4).map((h,i)=>(
            <div key={`b-${i}`} title={h?.name || 'ban'}>
              <div style={{
                width:22, height:22, borderRadius:6, background:'#2a2d3c', overflow:'hidden',
                outline:'2px solid #ea1c24'
              }}>
  ...
  function renderLegend(){
    return (
      <div style={{
        position:'absolute', top:16, left:16, background:'#23263a',
  ...
  function handleHeroClick(hero) {
    ...
    const isFinalBO7 = boType==='BO7' && currentGame===BO_OPTIONS.BO7;
  ...
  useEffect(() => {
    ...
    setPreChoose({ A: [null, null, null, null, null], B: [null, null, null, null, null] });
    setSelectingPre(null);
  },[firstPickTeam,teamARole, draftMode]);
