...
const renderHeroImage = (src, alt) => (
  <img
    ...
    onError={e => { e.target.onerror = null; e.target.src = `${process.env.PUBLIC_URL}/heroimages/default.jpg`; }}
  />
);
...
export default function TierData({ comboStats, games = [], dropdownDates = [] }) {
  ...
  const allOneHeroSplitted = useMemo(() => {
    return games.flatMap((g) =>
      g.fsPick?.flatMap((hero, i) => {
        ...
        if (!hero || !pos) return [];
        return [{ hero, position: pos, win: g.win === 1 || g.win === "1", total: 1, date: g.Date }];
      }) || []
    );
  }, [games]);
  ...
  const heroList = useMemo(() => {
    return uniqueHeroPositions.map((key) => {
      ...
      const evs = allOneHeroSplitted.filter((e) => e.hero === heroName && e.position === position);
  ...
  const combo2List = useMemo(() => {
       return (comboStats.combo2 || []).map((c) => {
         ...
         return {
           name:        c.combo,
           comboType,               // ใช้จาก parser โดยตรง
           characters:  chars,
  ...
  function filterByPeriodAndTeam(arr) {
    ...
    if (dateType === "day") {
      if (selectedDates.length) {
        ...
        res = res.filter((g) => g.Date >= selectedDateRange.from && g.Date <= selectedDateRange.to);
      }
    } else {
  ...
  const weekOptions = useMemo(
    () => [...new Set(Object.values(weekMap || {}))].sort((a, b) => +a.slice(1) - +b.slice(1)),
    [weekMap]
  );
